...
class TextSQL:
    ...
    With a custom / on-prem OpenAI-compatible endpoint (e.g. vLLM):
        engine = TextSQL(
            "sqlite:///mydb.db",
            model="openai:Qwen/Qwen3.6-35B-A3B",
            base_url="https://ibm-models.elsewedy-ec.com/v1",
            verify_ssl=False,
    ...
    def write_sql(
        ...
        """Single-pass SQL generation — no tool loop, faster, cheaper.

        Dumps the full schema into the prompt and asks the LLM to produce SQL
        in one call. No self-correction; accuracy degrades on large schemas.
